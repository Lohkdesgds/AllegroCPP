...
namespace AllegroCPP {
	...
	public:
		...
		void operator()(ALLEGRO_FONT* f) { 
			...
			vc.push_back(f);
			for (ALLEGRO_FONT* currdown = al_get_fallback_font(f); currdown != nullptr; currdown = al_get_fallback_font(f))
	...
	static _font_recursive_releaseable_deleter* get_destr(std::shared_ptr<ALLEGRO_FONT>& s) {
		return std::get_deleter<_font_recursive_releaseable_deleter>(s);
	}
	...
	bool Font::_load_autotype(const std::string& pth, const int res_x, const int res_y, int flg)
	{
		...
		if (res_x == res_y) {
			...
			{
				m_font = std::shared_ptr<ALLEGRO_FONT>(al_load_ttf_font(pth.c_str(), res_x, flg), _font_recursive_releaseable_deleter());
				if (m_font) return true;
				m_font = std::shared_ptr<ALLEGRO_FONT>(al_load_font(pth.c_str(), res_x, flg), _font_recursive_releaseable_deleter());
				if (m_font) return true;
			}
			else {
				m_font = std::shared_ptr<ALLEGRO_FONT>(al_load_font(pth.c_str(), res_x, flg), _font_recursive_releaseable_deleter());
			...
			m_font = std::shared_ptr<ALLEGRO_FONT>(al_load_ttf_font_stretch(pth.c_str(), res_x, res_y, flg), _font_recursive_releaseable_deleter());
			if (m_font) return true;
		}
		return false;
	}
	...
	Font::Font(ALLEGRO_FONT* eat)
		: m_font(std::shared_ptr<ALLEGRO_FONT>(eat, _font_recursive_releaseable_deleter()))
	{
	}
	Font::Font(std::shared_ptr<ALLEGRO_FONT> dup, draw_props prop)
		: m_stored_draw_props(prop), m_font(dup)
	{
	}
	...
	Font::Font(const int resolution, File&& ttffile, const int flags)
	{
		...
		if (!(m_font = std::shared_ptr<ALLEGRO_FONT>(al_load_ttf_font_f(drop->release(), fpnam.c_str(), resolution, flags), _font_recursive_releaseable_deleter()))) {
	...
	Font::Font(const int resolution_x, const int resolution_y, File&& ttffile, const int flags)
	{
		...
		File_shareable_ptr drop = ttffile;
		if (!(m_font = std::shared_ptr<ALLEGRO_FONT>(al_load_ttf_font_stretch_f(drop->release(), fpnam.c_str(), resolution_x, resolution_y, flags), _font_recursive_releaseable_deleter()))) {
			throw std::runtime_error("Cannot load ttf font file");
		}
	}
	...
	Font::Font(ALLEGRO_BITMAP* glyphbmpautocopy, const std::vector<font_ranges_pair> ranges)
	{
		...
		if (!(m_font = std::shared_ptr<ALLEGRO_FONT>(al_grab_font_from_bitmap(glyphbmpautocopy, static_cast<int>(ranges.size()), (int*)ranges.data()), _font_recursive_releaseable_deleter()))) {
	...
	Font::Font(const std::string& glyphpath, const std::vector<font_ranges_pair> ranges)
	{
		...
		// this copy bmp, it's safe
		if (!(m_font = std::shared_ptr<ALLEGRO_FONT>(al_grab_font_from_bitmap(bbmp.get(), static_cast<int>(ranges.size()), (int*)ranges.data()), _font_recursive_releaseable_deleter()))) {
			throw std::runtime_error("Cannot load glyph font");
		}
	}
	...
	Font::Font(const std::string& bmppath, const int flags)
	{
		...
		if (!(m_font = std::shared_ptr<ALLEGRO_FONT>(al_load_bitmap_font_flags(bmppath.c_str(), flags), _font_recursive_releaseable_deleter()))) {
	...
	Font::Font()
	{
		...
		if (!(m_font = std::shared_ptr<ALLEGRO_FONT>(al_create_builtin_font(), _font_recursive_releaseable_deleter()))) {
	...
	Font Font::make_ref() const
	{
		return Font(m_font, m_stored_draw_props);
	}
	...
	bool Font::push_fallback(Font&& oth)
	{
		...
		auto* deleter = get_destr(oth.m_font);
		deleter->release(); // m_font won't delete it anymore
		ALLEGRO_FONT* to_put = oth.m_font.get();

		ALLEGRO_FONT* it = m_font.get();
		ALLEGRO_FONT* test = nullptr;
		while ((test = al_get_fallback_font(it))) it = test;
		al_set_fallback_font(it, to_put);
		oth.m_font = nullptr;
		return true;
	}
	...
	size_t Font::fallback_count() const
	{
		if (!m_font) return 0;
		size_t c = 0;
		ALLEGRO_FONT* it = m_font.get();
	...
	{
		return m_font ? al_get_font_line_height(m_font.get()) : -1;
	}

	int Font::get_ascent() const
	{
		return m_font ? al_get_font_ascent(m_font.get()) : -1;
	}

	int Font::get_descent() const
	{
		return m_font ? al_get_font_descent(m_font.get()) : -1;
	}

	int Font::get_width(const char* str)
	{
		return m_font ? al_get_text_width(m_font.get(), str) : -1;
	}

	int Font::get_width(const std::string& str)
	{
		return m_font ? al_get_text_width(m_font.get(), str.c_str()) : -1;
	}

	int Font::get_width(ALLEGRO_USTR const* str)
	{
		return m_font ? al_get_ustr_width(m_font.get(), str) : -1;
	}

	int Font::get_glyph_width(int codepoint)
	{
		return m_font ? al_get_glyph_width(m_font.get(), codepoint) : -1;
	}
	...
	font_text_dimensions Font::get_dimensions(const char* str)
	{
		...
		al_get_text_dimensions(m_font.get(), str, &fd.x, &fd.y, &fd.w, &fd.h);
		return fd;
	}
	...
	font_text_dimensions Font::get_dimensions(const std::string& str)
	{
		if (!m_font) return { -1,-1,-1,-1 };
		font_text_dimensions fd{};
		al_get_text_dimensions(m_font.get(), str.c_str(), &fd.x, &fd.y, &fd.w, &fd.h);
		return fd;
	}
	...
	font_text_dimensions Font::get_dimensions(ALLEGRO_USTR const* str)
	{
		if (!m_font) return { -1,-1,-1,-1 };
		font_text_dimensions fd{};
		al_get_ustr_dimensions(m_font.get(), str, &fd.x, &fd.y, &fd.w, &fd.h);
		return fd;
	}
	...
	font_text_dimensions Font::get_glyph_dimensions(int codepoint)
	{
		if (!m_font) return { -1,-1,-1,-1 };
		font_text_dimensions fd{};
		al_get_glyph_dimensions(m_font.get(), codepoint, &fd.x, &fd.y, &fd.w, &fd.h);
		return fd;
	}

	int Font::get_glypth_advance(int codepoint, int codepoint2)
	{
		return m_font ? al_get_glyph_advance(m_font.get(), codepoint, codepoint2) : 0;
	}
	...
	bool Font::draw() const
	{
		...
		if (m_stored_draw_props._multiline) {
			...
			al_do_multiline_ustr(m_font.get(), max_width, _str, &__do_text_ustr_multiline, &_ref);
		}
		else {
			if (m_stored_draw_props._align == text_alignment::JUSTIFIED)
			{
				const auto& sett = m_stored_draw_props._justified_props;
				al_draw_justified_ustr(m_font.get(), m_stored_draw_props._color, m_stored_draw_props._pos.target_x, sett.max_x, m_stored_draw_props._pos.target_y, sett.diff, sett.extra_flags, _str);
			}
			else
			{
				al_draw_ustr(m_font.get(), m_stored_draw_props._color, m_stored_draw_props._pos.target_x, m_stored_draw_props._pos.target_y, static_cast<int>(m_stored_draw_props._align), _str);
			}
		}
	...
	bool Font::draw_glyph(float target_x, float target_y, int codepoint, ALLEGRO_COLOR color)
	{
		...
		al_draw_glyph(m_font.get(), color, target_x, target_y, codepoint);
		return true;
	}

	std::vector<font_ranges_pair> Font::get_ranges(int max)
	{
		int totalranges = max < 0 ? al_get_font_ranges(m_font.get(), 0, nullptr) : max;
		if (totalranges <= 0) return {};
		std::vector<font_ranges_pair> vec(static_cast<size_t>(max), {0,0});

		al_get_font_ranges(m_font.get(), static_cast<int>(vec.size()), (int*)vec.data());
		return vec;
	}
	...
	bool __do_text_ustr_multiline(int line_num, const ALLEGRO_USTR* line, void* extra)
	{
		...
		if (s->align == text_alignment::JUSTIFIED) { // JUST
			...
			al_draw_justified_ustr(s->font.get(), s->color, s->target_x, sett.max_x, y, sett.diff, sett.extra_flags, line);
		}
		else { // LEFT, CENTER, RIGHT
			al_draw_ustr(s->font.get(), s->color, s->target_x, y, static_cast<int>(s->align), line);
		}
