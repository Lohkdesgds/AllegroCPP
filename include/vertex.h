...
namespace AllegroCPP {
	...
	struct vertex_point : public ALLEGRO_VERTEX {
		...
		/// <param name="{ALLEGRO_COLOR}">Color of this point.</param>
		vertex_point(const float, const float, const ALLEGRO_COLOR&);
		...
		/// <param name="{ALLEGRO_COLOR}">Color of this point.</param>
		vertex_point(const float, const float, const float, const ALLEGRO_COLOR&);
		...
		/// <param name="{ALLEGRO_COLOR}">Color of this point.</param>
		vertex_point(const float, const float, const float, const float, const float, const ALLEGRO_COLOR&);
	};
	...
	public:
		...
		/// <returns>{Vertexes&amp;} Itself.</returns>
		Vertexes& push_back(const vertex_point&);
		...
		/// <param name="{vertex_point}">A vertex point to add.</param>
		/// <returns>{Vertexes&amp;} Itself.</returns>
		Vertexes& push_back(vertex_point&&);
		...
		/// <returns>{Vertexes&amp;} Itself.</returns>
		Vertexes& push_back(std::initializer_list<vertex_point>);

		/// <summary>
		/// <para>Set a texture to the triangles/lines</para>
		/// </summary>
		/// <param name="{Bitmap}">A hybrid memory to a texture.</param>
		/// <returns>{Vertexes&amp;} Itself.</returns>
		Vertexes& set_texture(const Bitmap&);
		...
		/// <param name="{float}">Move in Z.</param>
		void translate(const float, const float, const float = 0.0f);
	};
